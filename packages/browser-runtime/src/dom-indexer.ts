...
export function formatSnapshotForLLM(
  ...
  optionsOrMaxLength: number | { maxElements?: number; compact?: boolean; actionableOnly?: boolean; maxLength?: number } = 6000,
): string {
  // Support both old (number) and new (object) call signatures
  let maxLength = 6000;
  let maxElements = Infinity;
  if (typeof optionsOrMaxLength === "number") {
    maxLength = optionsOrMaxLength;
  } else {
    maxLength = optionsOrMaxLength.maxLength ?? 6000;
    maxElements = optionsOrMaxLength.maxElements ?? Infinity;
  }
  ...
  for (const el of snapshot.elements) {
    if (count >= maxElements) {
      const remaining = snapshot.elements.length - count;
      if (remaining > 0) {
        lines.push(`... and ${remaining} more elements (limit: ${maxElements})`);
      }
      break;
    }
    count++;
